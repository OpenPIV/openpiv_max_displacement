...
#    '''the func fuction is the "frame" in which the PIV evaluation is done'''
    ...
    def func(args):
        ...
        files_a =sorted( glob.glob( os.path.join( os.path.abspath(settings.filepath_images), settings.frame_pattern_a ) ) )
        n_Pairs=len(files_a)
        # Define the zero pads
        if n_Pairs<10:
            String_PAD='02d'
        elif n_Pairs<100:
            String_PAD='03d'
       ...
       # counter = settings.counter
        ...
        # if settings.dynamic_masking_method=='edges' or 'intensity':    
...
#                v,dummy_v1,dummy_v2,dummy_v3=smoothn(v,s=settings.smoothn_p)   
        ...
        'save to a file'
        tools_patch.save_windef(x, y, u, v,sig2noise_ratio, mask ,os.path.join(save_path,str('field_%'+String_PAD+'.txt') % counter), delimiter='\t')
        ...
        if settings.show_plot==True or settings.save_plot==True:
           # plt.close('all')
            fig, ax = tools_patch.display_vector_field_windef(os.path.join(save_path, str('field_%'+String_PAD+'.txt') % counter), scaling_factor=settings.scale_plot)
            Name = os.path.join(save_path, str('Field%'+String_PAD+'.png') % (counter))
